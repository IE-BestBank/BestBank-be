...
# Default environment to 'local' and fetch config based on environment variable
...
# **Step 1: Set up OpenTelemetry (if enabled in config)**
...
if app.config.get('APPINSIGHTS_INSTRUMENTATIONKEY'):
...
def test_monitoring():
    try:
        ...
        if telemetry_client:
            ...
            logger.warning("TelemetryClient is not configured")
